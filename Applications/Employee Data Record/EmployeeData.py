...
class EmployeeData:
    def __init__(self, window):
        self.connection = mysql.connect(host="localhost", user="root", password="2323", database="contactlistdb")
        self.wind = window
        self.wind.title('Contacts List Application')


        mainframe = Frame(window, background="pink")
        mainframe.place( x=0, y=10, anchor="nw", width=250, height=300)

        addBtn=Button(mainframe ,text="ADD", command=self.insert_contact, width=25)
        addBtn.place(x=25,y=30)

        modifyBtn=Button(mainframe, text="MODIFY", command=self.modify_contact, width=25)
        modifyBtn.place(x=25,y=70)

        displayBtn=Button(mainframe, text="DISPLAY", command=self.display_contact, width=25)
        displayBtn.place(x=25, y=110)

        delBtn=Button( text="DELETE", command=self.delete_contact, width=25 )
        delBtn.place(x=25,y=160)

        frame = LabelFrame( mainframe, text="Search a String" )
        frame.place(x=10,y=210)

        Label( frame, text="Search ContactDB: " ).grid( row=5, column=0 )
        self.search_string = Entry( frame )
        self.search_string.grid( row=5, column=1)
        searchBtn=Button(mainframe, text="Search", command=self.search_contact, width=25)
        searchBtn.place(x=25,y=260)




        self.message = Label( text="", fg="red" )
        self.message.place(x=150,y=325)

        siderFame=Frame(window)
        siderFame.place(x=250,y=10,width=1250,height=300)

        s = ttk.Style()
        s.configure('Treeview', rowheight="25" )
        s.configure( "Treeview", foreground='red' )
        s.configure( "Treeview.Heading", foreground='green' )
        self.tree = ttk.Treeview(siderFame, height=11, columns=(
            'FirstName', 'MiddleName', 'LastName', 'Address List', 'PhoneList', 'DateList') )

        self.tree.place(x=10,y=0)
    ...
    def create_scrollbar(self):
        ...
        #self.scrollbar.grid( row=2, column=15, rowspan=10, sticky='sn' )
    ...
    def search_contact(self):
        ...
        query =  "SELECT c.Contact_id, c.Fname, c.Lname, c.Email FROM CONTACT c, ADDRESS a, PHONE p, DATE d WHERE " \
                 "c.Contact_id = a.Contact_id and a.Contact_id = p.Contact_id and p.Contact_id = d.Contact_id and " \
                 "CONCAT(c.Contact_id, '', c.Fname, '', c.Lname, '', c.Email, '', a.Address_id, '', a.Address_type, " \
                 "'', a.Address, '', a.City, '', a.State, '', a.Zip, '', p.Phone_id, p.Phone_type, p.Area_code, " \
                 "p.Number, d.Date_id, d.Date_type, d.Date) LIKE '%{}%'".format(self.search_string.get())
    ...
    def add_contact(self):
        if self.validation():
            ...
            email_str = self.email.get()
            query = "INSERT INTO CONTACT(Fname, Lname, Email) VALUES('" + fname_str + "','" + "','" + lname_str + "','"+email_str + "')"
            self.run_query(query, "INSERT")
            self.message['text'] = "Contact {} {} {} has been added successfully".format(fname_str, lname_str, email_str,)
    ...
    def modify_contact(self):
        ...
        try:
            ...
            old_lname_str = self.tree.item(self.tree.selection())['values'][1]
            old_email_str = self.tree.item( self.tree.selection() )['values'][2]
        except IndexError as e:
            self.message['text'] = "Please select a Record"
            return
        ...
        # Old Middle Name
        Label(self.edit_wind, text=" Old Last Name:").grid(row=5, column=1)
        Entry(self.edit_wind, textvariable=StringVar(self.edit_wind, value=old_lname_str), state="readonly").grid(row=5,
                                                                                                                  column=2)
        # New Middle Name
        Label(self.edit_wind, text="New Last Name:").grid(row=6, column=1)
        new_lname_str= Entry(self.edit_wind)
        new_lname_str.grid(row=6, column=2)

        # Old Last Name
        Label(self.edit_wind, text="Old Email Address: ").grid(row=7, column=1)
        Entry(self.edit_wind, textvariable=StringVar(self.edit_wind, value=old_email_str), state="readonly").grid(row=7,
                                                                                                                  column=2)
        # New Last Name
        Label(self.edit_wind, text="New Email Address: ").grid(row=8, column=1)
        new_email_str = Entry(self.edit_wind)
        new_email_str.grid(row=8, column=2)
        ...
        Button(self.edit_wind, text="Update",
               command=lambda: self.edit_contacts(new_Contact_id_str, new_fname_str.get(), new_email_str.get(),
    ...
    def edit_contacts(self, new_Contact_id_str, new_fname_str, new_lname_str, new_email_str, Contact_id_str,
        ...
        try:
            if new_fname_str != "" and new_lname_str != "":
                print(new_fname_str, new_lname_str)
                query = "UPDATE CONTACT SET Contact_id={}, Fname='{}', Lname='{}, Email='{}',' WHERE Contact_id='{}'".format(
                    new_Contact_id_str, new_fname_str,  new_lname_str, new_email_str,Contact_id_str)
    ...
    def insert_contact(self):
        ...
        Label(self.add_wind, text="Last Name: ").grid(row=3, column=1)
        lname_str = Entry(self.add_wind)
        lname_str.grid(row=3, column=2)

        Label(self.add_wind, text="Email: ").grid(row=4, column=1)
        email_str = Entry(self.add_wind)
        email_str.grid(row=4, column=2)
        ...
        try:
            Button(self.add_wind, text="Confirm",
                   command=lambda: self.insert_contact_db(Contact_id_str.get(), fname_str.get(), lname_str.get(),
                                                          email_str.get(), addrtype_str.get(), addr_str.get(),
    ...
    def insert_contact_db(self, Contact_id_str, fname_str,  lname_str, email_str, addrtype_str, addr_str, city_str,
                          state_str, zip_str, ptype_str, areacode_str, number_str, dtype_str, date_str):

        if Contact_id_str != "":
            if fname_str != "" and lname_str != "":
                query = "INSERT INTO CONTACT(Contact_id, Fname, Lname, Email) VALUES('" + Contact_id_str + "','" + fname_str + "','" + lname_str  + "','" + email_str + "')"
                self.run_query(query, "INSERT")
            ...
            if fname_str != "" and lname_str != "":
                ...
                get_query = "SELECT Contact_id FROM CONTACT WHERE Fname = '" + fname_str + "' and Lname = '" + lname_str + "' and Email = '" + email_str + "'"
        ...
        self.message['text'] = "Contact {} {} Added Successfully".format(fname_str, lname_str)
...
if __name__ == "__main__":
    window = Tk()
    window.geometry("1480x360")
    application = EmployeeData(window)
    window.mainloop()